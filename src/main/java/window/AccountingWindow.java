...
//        List<Category> allCategoriesFromDataBase = CategoryService.getAllCategoriesInSystem(entityManagerFactory, accountingSystem);
//        for (int i=0;i<allCategoriesFromDataBase.size();i++) {
//            if(allCategoriesFromDataBase.get(i).getParentCategory() == null /*&& allCategoriesFromDataBase.get(i).getResponsibleUsers().contains(activeUser)*/){
//                for(int j=0; j<allCategoriesFromDataBase.get(i).getResponsibleUsers().size();j++)
//                {
//                    if(allCategoriesFromDataBase.get(i).getResponsibleUsers().get(j).getId() == activeUser.getId())
//                    {
//                        categoryList.getItems().add(allCategoriesFromDataBase.get(i).getTitle());
//                        break;
//                    }
//                }
//
